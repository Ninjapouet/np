...
*)
 ...
 let () =
  (* pipe *)
  let left, send_left, right, send_right = pipe () in
  let left_effect = E.map (Fmt.pr "%s@.") left in
  let right_effect = E.map (Fmt.pr "%s@.") right in
  send_left "ping";
  send_right "pong";
...
let () = Lwt_main.run begin
    ...
    let output_a, send_output_a = E.create () in
    let output_b, send_output_b = E.create () in

    let a_ip, a_port = Unix.inet_addr_loopback, 3000 in
    let b_ip, b_port = Unix.inet_addr_loopback, 3001 in

    let input_a = Np.Udp.Net.make a_ip a_port output_a in
    let input_b = Np.Udp.Net.make b_ip b_port output_b in

    let a_effect = E.map (fun (_, _, msg) -> Fmt.pr "a: %s@." msg) input_a in
    let b_effect = E.map (fun (_, _, msg) -> Fmt.pr "b: %s@." msg) input_b in

    send_output_a (b_ip, b_port, "A");

    Lwt_unix.sleep 0.1;%lwt

    send_output_b (a_ip, a_port, "B");

    (* wait a bit for messages *)
    Lwt.pick [
      Lwt_unix.sleep 1.0;
      begin
        let%lwt _ = E.next a_effect in
        let%lwt _ = E.next b_effect in
        Lwt.return_unit
      end;
    ];%lwt
